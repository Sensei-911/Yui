...
const random = await misc.random(1, number);
...
msg = `SMH it's gotta be a **valid** number between \`1\` and \`${number}\`\n`;
} else if (picked > number || picked < 1) {
msg = `Listen buddy, it's gotta be a number between \`1\` and \`${number}\`. No higher, no lower\n`;
} else {
msg = `not this time, `;
}
