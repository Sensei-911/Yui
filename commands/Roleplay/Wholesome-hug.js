...
module.exports = {
name: 'hug',
description: 'Hug specified member.',
execute(client, message, args) {

const user = message.mentions.users.first()
const image = images[Math.floor(Math.random() * images.length)]
const hugmessage = args.slice(1).join(' ')

if (!user) {
let hug_embed = new Discord.MessageEmbed()
.setDescription(`**${message.author.username}** hugs himself/herself :(`)
.setColor('RED')
.setImage(image)
message.channel.send(hug_embed);
return
}
if (hugmessage.length > 0) {
let hug_embed = new Discord.MessageEmbed()
.setDescription(`**${message.author.username}** hugs **<@!${user.id}>**\n\"${hugmessage}\"`)
.setColor('RED')
.setImage(image)
message.channel.send(hug_embed);
return
...
let hug_embed = new Discord.MessageEmbed()
.setDescription(`**${message.author.username}** hugs **me** uwu <3`)
.setColor('RED')
.setImage(image)
message.channel.send(hug_embed);
return
