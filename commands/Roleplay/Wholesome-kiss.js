...
module.exports = {
name: 'kiss',
description: 'Kiss specified member.',
execute(client, message, args) {
    
const user = message.mentions.users.first()
const image = images[Math.floor(Math.random() * images.length)]
const kissmessage = args.slice(1).join(' ')
    
if (!user) {
let kiss_embed = new Discord.MessageEmbed()
.setDescription(`**${message.author.username}** somehow kisses himself/herself wow`)
.setColor('RED')
.setImage(image)
message.channel.send(kiss_embed);
return
}
if (kissmessage.length > 0) {
let kiss_embed = new Discord.MessageEmbed()
.setDescription(`**${message.author.username}** kisses **<@!${user.id}>**\n\"${kissmessage}\"`)
.setColor('RED')
.setImage(image)
message.channel.send(kiss_embed);
return
...
let kiss_embed = new Discord.MessageEmbed()
.setDescription(`**${message.author.username}** kisses **me** what the heck`)
.setColor('RED')
.setImage(image)
message.channel.send(kiss_embed);
return
